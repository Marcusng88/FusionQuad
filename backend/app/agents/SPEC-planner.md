...
High-level strategy agent that reads RAG-backed guideline files and sets the `optimization_strategy` dict consumed by the Optimization node. Analyzes forecast context, day type, tariff window, and BESS state to produce a situational strategy with rationale.
...
```
[Forecasting] → [Tariff] → [Planner] → [Optimization] → [Controller] → [Auditor]
```

Planner runs **every simulation tick** after Tariff, before Optimization.
